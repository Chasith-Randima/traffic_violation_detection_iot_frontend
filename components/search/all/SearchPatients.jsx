...
const SearchPatients = () => {
  ...
  const handleSubmit = async () => {
    ...
    await searchPatients({ search: search }, token)
      .then((data) => {
        console.log(data);
        setData(data);
        // console.log(data);
                ...
                <>
                  <div className="grid grid-cols-5 p-4 border-b-2 border-gray-200 hover:bg-gray-300 transition-all rounded-xl text-center">
                    <h3 className="hidden md:block col-span-1">
                      {patient._id.substr(
                        patient._id.length / 2,
                        patient._id.length
                      )}
                    </h3>
                    <h3 className="md:hidden col-span-1">
                      {patient._id.substr(
                        patient._id.length - 5,
                        patient._id.length
                      )}
                    </h3>
                    ...
                    <h3 className="">{patient.nic}</h3>
                    <h3 className="">{patient.dateOfBirth.split("T")[0]}</h3>
                    <h3 className="">{patient.city}</h3>
                    {/* <h3></h3> */}
                    {/* <h3 className="col-span-2">
                      {patient.active ? "Active" : "Closed"}
                    </h3> */}
                  </div>
                </>
              );
