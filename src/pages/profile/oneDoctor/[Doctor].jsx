...
const Doctor = ({ router }) => {
  ...
  const { name, email, doctorId, images, formData, hospitals } = values;
  useEffect(() => {
        ...
        .then((data) => {
          if (data.status && data.status == "success") {
            ...
            setValues({
              ...
              hospitals:
                data.doc.hospitals.length > 0 ? data.doc.hospitals[0] : "",
  ...
  const handleSubmit = async (e) => {
    ...
    await updateDoctor(router.query.Doctor, values.formData, token)
      .then((data) => {
        if (data.status && data.status == "success") {
          console.log(data);
          setAlert({
  ...
  const handleAuthSubmit = async (e) => {
    ...
    let id = router.query.Doctor;

    await updateDoctorPassword(id, data, token)
      .then((data) => {
        if (data.status && data.status == "success") {
          console.log(data);
          // return data;
